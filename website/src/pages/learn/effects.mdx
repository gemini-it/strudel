...
Comme vous pouvez le soupçonner, les effets peuvent être chaînés ensemble, et ils acceptent une chaîne de pattern comme argument.
...
- Un événement générateur de son est déclenché par un pattern
...
- Ce qui suit se produira _dans l'ordre_ et seulement s'ils ont été appelés dans le pattern. Notez que tous ces
  effets à usage unique, ce qui signifie que plusieurs occurrences d'entre eux dans un pattern remplaceront simplement les valeurs
...
- La sortie sèche, le delay et la reverb sont joints dans ce qu'on appelle l'"orbite" du pattern (voir plus dans la section ci-dessous)
  - L'effet `duck` affecte le volume de tous les signaux dans l'orbite
  - L'orbite est ensuite envoyée au mixeur

## Orbites

Les orbites sont la façon dont les sorties sont gérées dans Strudel. Elles prescrivent également quel delay et reverb associer au signal sec.
Par défaut, toutes les orbites sont mixées vers les canaux `1` et `2` en stéréo, cependant avec le paramètre "Multi Channel Orbits"
...
L'orbite par défaut est `1` et elle est définie avec `orbit`. Vous pouvez envoyer un son vers plusieurs orbites via mininotation
...
⚠️ Il n'y a qu'un seul delay et reverb par orbite, alors soyez conscient que si vous tentez de changer les paramètres sur deux
patterns pointant vers la même orbite, cela peut conduire à des résultats imprévisibles. Comparez, par exemple, ce joli pluck
avec une grande reverb :
...
Ne produira pas un filtre passe-bas continuellement LFO'd en raison du `tri` n'étant échantillonné que chaque fois que la note frappe
...
- cutoff : la fréquence à laquelle le filtre commence à fonctionner. par exemple, un filtre passe-bas avec une coupure de 1000Hz permet aux fréquences inférieures à 1000Hz de passer.
...
Vous créez également de jolis sons de style chiptune :
