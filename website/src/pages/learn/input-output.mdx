...
Normalement, Strudel est utilisé pour créer des motifs sonores, en utilisant son propre synthétiseur basé sur '[web audio](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API)' appelé [SuperDough](https://codeberg.org/uzu/strudel/src/branch/main/packages/superdough).

Il est également possible de créer des motifs pour d'autres éléments avec Strudel, comme des synthétiseurs logiciels et matériels avec MIDI, d'autres logiciels utilisant Open Sound Control/OSC (y compris le synthétiseur [SuperDirt](https://github.com/musikinformatik/SuperDirt/) couramment utilisé avec le frère de Strudel [TidalCycles](https://tidalcycles.org/)), ou le protocole 'Internet des objets' MQTT.

# MIDI

Strudel prend en charge MIDI sans aucun logiciel supplémentaire (grâce à [webmidi](https://npmjs.com/package/webmidi)), simplement en ajoutant des méthodes à votre motif :
...
Connectez soit un périphérique MIDI, soit utilisez le pilote IAC (Mac) ou le port Midi Through (Linux) pour les messages MIDI internes.
Si aucun outputName n'est fourni, il utilise la première sortie MIDI qu'il trouve.
...
Dans la console, vous verrez un journal des périphériques MIDI disponibles dès que vous exécutez le code,
par exemple
...
| ------------ | ------------- | --------- | ----------------------------------------------------------------------------- |
| isController | booléen       | false     | Lorsque vrai, désactive l'envoi de messages de notes. Utile pour contrôleurs MIDI |
| latencyMs    | nombre        | 34        | Latence en millisecondes pour aligner MIDI avec le moteur audio               |
| noteOffsetMs | nombre        | 10        | Décalage en millisecondes pour les messages note-off pour éviter les glitchs  |
| midichannel  | nombre        | 1         | Canal MIDI par défaut (1-16)                                                   |
| velocity     | nombre        | 0.9       | Vélocité de note par défaut (0-1)                                              |
| gain         | nombre        | 1         | Multiplicateur de gain par défaut pour la vélocité (0-1)                       |
| midimap      | chaîne        | 'default' | Nom du mappage MIDI à utiliser pour les changements de contrôle               |
| midiport     | chaîne/nombre | -         | Nom ou index du périphérique MIDI                                              |

</details>

### midiport(outputName)

Sélectionne le périphérique de sortie MIDI à utiliser, le motif peut être utilisé pour basculer entre les périphériques.
...
Sélectionne le canal MIDI à utiliser. Si non spécifié, `.midi` utilisera le canal 1 par défaut.
...
- `ccn` définit le numéro de CC. Dépend du mappage MIDI de votre synthétiseur.
- `ccv` définit la valeur de CC, normalisée de 0 à 1.
...
Dans l'extrait ci-dessus, `ccn` est défini sur 74, qui est la coupure de filtre pour de nombreux synthétiseurs. `ccv` est contrôlé par un motif en dents de scie.
Ayant tout dans un motif, le motif `ccv` sera aligné avec le motif de notes, car la structure vient de la gauche par défaut.
...
Au lieu de définir `ccn` et `ccv` directement, vous pouvez également créer des mappages avec `midimaps` :
...
`progNum` envoie des messages de changement de programme MIDI pour basculer entre différents préréglages/patchs sur votre périphérique MIDI.
...
Les messages de changement de programme sont utiles pour basculer entre différents sons d'instruments ou préréglages pendant une performance.
...
La valeur doit être un tableau de nombres entre 0 et 255 représentant les octets de données SysEx.
...
`midibend` définit la plage de pitch bend MIDI (-1 - 1)
`miditouch` définit le key aftertouch MIDI (0-1)
...
1. installer SuperCollider + plugins sc3, voir [Tidal Docs](https://tidalcycles.org/docs/) (Installer Tidal) pour plus d'informations.
...
4. télécharger le [dépôt Strudel](https://codeberg.org/uzu/strudel/) (ou cloner le dépôt git, si vous avez git installé)
...
1. Démarrez SuperCollider, soit en utilisant l'IDE SuperCollider, soit en exécutant `sclang` dans un terminal.
...
Remarque : si vous avez la 'Cible du moteur audio' dans les paramètres définie sur 'OSC', vous n'avez pas besoin d'ajouter .osc() à la fin de votre motif.
...
MQTT est un protocole réseau léger, conçu pour les appareils de l'« Internet des objets ». Pour une utilisation avec Strudel, vous aurez
...
en exécuter un vous-même (par exemple en exécutant [mosquitto](https://mosquitto.org/)), bien qu'obtenir un certificat SSL auquel
...
L'exemple suivant montre comment envoyer un motif à un broker MQTT :
...
Les motifs de contrôle seront encodés en JSON, par exemple :
