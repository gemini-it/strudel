...
Prenons du recul et comprenons comment fonctionne la syntaxe dans Strudel.

Regardez cet exemple simple :

<MiniRepl client:idle tune={`note("c a f e").s("piano")`} />

- Nous avons un mot `note` qui est suivi de parenthèses `()` avec des mots/lettres/nombres à l'intérieur, entourés de guillemets `"c a f e"`
...
La fonction `yyy` est appelée une fonction [_chaînée_](https://en.wikipedia.org/wiki/Method_chaining), car elle est précédée d'un point (`.`).
...
Strudel utilise massivement les fonctions chaînées. Voici un exemple plus sophistiqué :
...
Le `//` dans l'exemple ci-dessus est un commentaire de ligne, ce qui fait que la fonction `delay` est ignorée.
C'est un moyen pratique d'activer et de désactiver rapidement le code.
Essayez de décommenter cette ligne en supprimant `//` et en rafraîchissant le pattern.
...
Dans TidalCycles, les chaînes entre guillemets doubles sont utilisées pour écrire des _patterns_ en utilisant la mini-notation, et vous pourriez entendre l'expression _chaîne de pattern_ de temps en temps.
Si vous voulez créer une chaîne normale et pas un pattern, vous pouvez utiliser des guillemets simples, par exemple `'C minor'` ne sera pas analysé comme de la Mini Notation.

La bonne nouvelle, c'est que cela couvre la plupart de la syntaxe JavaScript nécessaire pour Strudel !
