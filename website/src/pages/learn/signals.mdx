---
title: Signaux
layout: ../../layouts/MainLayout.astro
---

import { MiniRepl } from '../../docs/MiniRepl';
import { JsDoc } from '../../docs/JsDoc';

# Signaux continus

Les signaux sont des patterns avec des valeurs continues, ce qui signifie qu'ils ont théoriquement un nombre infini d'étapes.
Ils peuvent fournir des flux de nombres qui peuvent être échantillonnés à des points discrets dans le temps.

## saw

<JsDoc client:idle name="saw" h={0} />

## sine

<JsDoc client:idle name="sine" h={0} />

## cosine

<JsDoc client:idle name="cosine" h={0} />

## tri

<JsDoc client:idle name="tri" h={0} />

## square

<JsDoc client:idle name="square" h={0} />

## rand

<JsDoc client:idle name="rand" h={0} />

## Plages de -1 à 1

Il existe également `saw2`, `sine2`, `cosine2`, `tri2`, `square2` et `rand2` qui ont une plage de -1 à 1 !

## perlin

<JsDoc client:idle name="perlin" h={0} />

## irand

<JsDoc client:idle name="irand" h={0} />

## brand

<JsDoc client:idle name="brand" h={0} />

## brandBy

<JsDoc client:idle name="brandBy" h={0} />

## mouseX

<JsDoc client:idle name="mousex" h={0} />

## mouseY

<JsDoc client:idle name="mousey" h={0} />

Ensuite : [Modificateurs aléatoires](/learn/random-modifiers)
