...
Notez que seules les cartes d'échantillons (mappant les noms aux URL) sont chargées initialement, tandis que les échantillons audio eux-mêmes ne sont pas chargés tant qu'ils ne sont pas réellement joués.
...
Vous pourriez même utiliser la banque pour basculer entre différentes boîtes à rythmes :
...
Les numéros qui sont trop élevés reviendront simplement au début.
...
## Chargement d'échantillons depuis des URL de fichiers
...
La méthode ci-dessus pour charger des échantillons peut être fastidieuse à écrire/copier-coller chaque fois que vous écrivez un nouveau motif.
...
## Depuis le disque via "Importer un dossier de sons"
...
Allez dans l'onglet `sounds` du REPL et ouvrez l'onglet `import-sounds` sous la barre de recherche.
Appuyez sur le bouton "importer un dossier de sons" et sélectionnez un dossier contenant des fichiers audio.
...
Au lieu de charger vos échantillons dans votre navigateur avec le bouton "importer un dossier de sons", vous pouvez également servir les échantillons depuis un serveur de fichiers local.
...
Notez que le code de langue et les paramètres de genre sont optionnels et par défaut `en-GB` (anglais britannique) et `f` (féminin).
