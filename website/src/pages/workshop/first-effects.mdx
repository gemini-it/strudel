...
- Changez lpf à 200. Remarquez comme cela devient étouffé. Pensez-y comme si vous étiez devant le club avec la porte fermée 🚪.
- Maintenant, ouvrons la porte... changez-le à 5000. Remarquez comme cela devient plus brillant ✨🪩
...
- Remarquez comment le pattern dans lpf ne change pas le rythme global
...
Combinons tout ce qui précède dans un petit morceau :
...
**façonner le son avec une enveloppe adsr**
...
Essayez de découvrir ce que font les nombres.. Comparez les éléments suivants

- attack (attaque) : `.5` vs `0`
- decay (décroissance) : `.5` vs `0`
- sustain (maintien) : `1` vs `.25` vs `0`
- release (relâchement) : `0` vs `.5` vs `1`
...
- attack : temps nécessaire pour le fondu d'entrée
- decay : temps nécessaire pour atteindre le niveau de sustain
- sustain : niveau après decay
- release : temps nécessaire pour le fondu de sortie après la fin de la note
...
Essayez quelques valeurs de `delay` entre 0 et 1. D'ailleurs, `.5` est l'abréviation de `0.5`

Que se passe-t-il si vous utilisez `.delay(".8:.125")` ? Pouvez-vous deviner ce que fait le deuxième nombre ?

Que se passe-t-il si vous utilisez `.delay(".8:.06:.8")` ? Pouvez-vous deviner ce que fait le troisième nombre ?
...
**room alias reverb (réverbération)**
...
Ajoutons une basse pour compléter cela :
...
Essayez d'ajouter `.hush()` à la fin d'un des patterns dans la pile...
...
Nous pouvons utiliser `fast` et `slow` pour changer le tempo d'un pattern en dehors de la Mini-Notation :
...
Que se passe-t-il si vous utilisez un pattern comme `.fast("<1 [2 4]>")` ?

</Box>

D'ailleurs, dans la Mini-Notation, `fast` correspond à `*` et `slow` correspond à `/`.

<MiniRepl client:visible tune={`sound("[bd*4,~ rim ~ cp]*<1 [2 4]>")`} />

## modulation avec des signaux

Au lieu de changer les valeurs par paliers, nous pouvons également les contrôler avec des signaux :

<MiniRepl client:visible tune={`sound("hh*16").gain(sine)`} punchcard punchcardLabels={false} />

<Box>

Les formes d'onde de base pour les signaux sont `sine`, `saw`, `square`, `tri` 🌊

Essayez aussi les signaux aléatoires `rand` et `perlin` !

Le gain est visualisé comme la transparence dans le pianoroll.

</Box>

**définir une plage**

Par défaut, les ondes oscillent entre 0 et 1. Nous pouvons changer cela avec `range` :
...
La modulation complète prendra maintenant 8 cycles pour se répéter.
...
Jetons maintenant un coup d'œil à certains [effets de pattern](/workshop/pattern-effects) typiques de Tidal.